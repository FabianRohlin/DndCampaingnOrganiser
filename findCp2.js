...
function FindDataCp2(){
    const dbref = ref(db);
    get(child(dbref, 'Sheets/campaign2/' + findID.value)).then((snapshot) => {
        if (snapshot.exists()) {
...
get(child(dbRef, 'Sheets/campaign2')).then((snapshot) => {
  if (snapshot.exists()) {
    let folderNames = '';
 ...
 function RemoveDataCp2(){
     var id = findID.value;

     remove(ref(db, 'Sheets/campaign2/' + id))
     .then(() => {
         alert("Data removed");
     })
     .catch((error) => {
...
findBtn.addEventListener("click", FindDataCp2);
removeBtn.addEventListener("click", RemoveDataCp2);